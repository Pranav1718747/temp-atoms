...
exports.default = exports.AirQualityPredictor = void 0;
...
class AirQualityLSTM {
    constructor() {
        this.weights = null;
        this.hiddenSize = 50;
        this.sequenceLength = 24; // 24 hours
    ...
    predictSequence(sequence, steps) {
        const predictions = [];
    ...
    initializeWeights() {
        ...
        this.weights = {
            input: ml_matrix_1.Matrix.random(this.hiddenSize, inputSize + this.hiddenSize),
            forget: ml_matrix_1.Matrix.random(this.hiddenSize, inputSize + this.hiddenSize),
            cell: ml_matrix_1.Matrix.random(this.hiddenSize, inputSize + this.hiddenSize),
            output: ml_matrix_1.Matrix.random(1, this.hiddenSize)
        };
    }
    forward(sequence) {
        if (!this.weights)
            throw new Error('Weights not initialized');
        let cellState = ml_matrix_1.Matrix.zeros(this.hiddenSize, 1);
        let hiddenState = ml_matrix_1.Matrix.zeros(this.hiddenSize, 1);
        // Process sequence
        for (let t = 0; t < Math.min(sequence.length, this.sequenceLength); t += 6) {
            ...
            const combined = ml_matrix_1.Matrix.columnVector([...inputMatrix.getRow(0), ...hiddenState.getColumn(0)]);
    ...
    updateWeights(learningRate) {
        ...
        this.weights.output = this.weights.output.add(ml_matrix_1.Matrix.random(1, this.hiddenSize).mul(learningRate));
    }
}
/**
...
class HealthImpactAssessor {
    ...
    calculateSubAQI(concentration, breakpoints) {
        for (const bp of breakpoints) {
            if (concentration >= bp.low && concentration <= bp.high) {
                return Math.round(((bp.aqiHigh - bp.aqiLow) / (bp.high - bp.low)) * (concentration - bp.low) + bp.aqiLow);
            }
        }
...
class AirQualityPredictor {
    ...
    calculateCurrentAQI(pollutants) {
    ...
    identifyDominantPollutant(pollutants) {
        const levels = [
    ...
    generateTrainingData() {
        ...
        for (let i = 0; i < numSamples; i++) {
            ...
            for (let hour = 0; hour < 24; hour++) {
                sequence.push(5 + Math.random() * 50, // PM2.5
...
exports.AirQualityPredictor = AirQualityPredictor;
exports.default = AirQualityPredictor;
//# sourceMappingURL=air-quality-predictor.js.map