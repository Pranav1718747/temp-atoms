...
const { initializeAdvancedML, getAdvancedWeatherPredictions, getAdvancedCropRecommendations, getAdvancedAlertPredictions, getComprehensiveInsights, getMLHealthStatus } = require('./ml-ts/bridge.js');
...
class ClimateServer {
    constructor() {
        ...
        this.config = {
            ...
            staticPath: path_1.default.join(__dirname, 'frontend')
        };
    ...
    initializeRoutes() {
        ...
        this.app.get('/', (req, res) => {
            res.sendFile(path_1.default.join(this.config.staticPath, 'index.html'));
        });
        // 404 handler
        ...
        this.io.on('connection', (socket) => {
            ...
            socket.on('subscribe_weather', (city) => {
                ...
                try {
                    ...
                    if (cityInfo) {
                        this.alertService.subscribeToAlerts(socket.id, cityInfo.id, city, ['FLOOD', 'HEAT', 'DROUGHT', 'STORM']);
            ...
            socket.on('unsubscribe_weather', (city) => {
                if (city && typeof city === 'string') {
    ...
    loadCitiesFromDB() {
        try {
            const allCities = this.climateDB.getAllCities();
     ...
     */
    async sendCurrentWeatherToSocket(socket, city) {
        try {
     ...
     */
    async fetchAndBroadcastWeather(city) {
        try {
